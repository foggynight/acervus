...
(define (parse-directive! tokens)
  ...
  (unless (null? (cdr tokens))
    (error "acervus: parser error: tokens trailing directive"))
  (unless (valid-directive? dir)
    (error "acervus: parser error: invalid directive"))
  (program-cons! dir))

(define (parse-label! tokens)
  (define lab (token/data->symbol (car tokens)))
  (when (assq lab %labels)
    (error "acervus: parser error: duplicate label"))
  (labels-cons! lab)
  (unless (null? (cdr tokens))
  ...
  (if (token-instruction? (car tokens))
      (let ((inst (token/instruction->symbol (car tokens))))
        (unless (= (length (cdr tokens)) (operand-count inst))
          (error "acervus: parser error: invalid number of operands"))
        (program-cons! inst)
        (for-each (lambda (tok)
                    (if (token-operand? tok)
                        (program-cons! (token/operand->value tok))
                        (error "acervus: parser error: invalid operand")))
                  (cdr tokens)))
      (if (null? (cdr tokens))
          (program-cons! (token/immediate->value (car tokens)))
          (error "acervus: parser error: tokens trailing immediate"))))
